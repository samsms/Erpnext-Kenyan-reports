...
def execute(filters=None):
	...
	for ss in salary_slips:
		...
		row = [
			ss.name, ss.employee, ss.employee_name, emp_det.date_of_joining, emp_det.national_id,
			emp_det.nssf_no, emp_det.nhif_no, emp_det.tax_id, ss.bank_name, ss.bank_account_no,
			ss.branch, ss.department, ss.designation, ss.company, ss.start_date, ss.end_date,
			ss.leave_without_pay, ss.payment_days
		]
		...
		if currency == company_currency:
			row.append(flt(ss.gross_pay) * flt(ss.exchange_rate))
		else:
		...
		if currency == company_currency:
			row.append(flt(ss.total_deduction) * flt(ss.exchange_rate))
			row.append(flt(ss.net_pay) * flt(ss.exchange_rate))
		else:
...
def get_columns(salary_slips):
	columns = [
		_("Salary Slip ID") + ":Link/Salary Slip:150", _("Employee") + ":Link/Employee:120",
		_("Employee Name") + "::140", _("Date of Joining") + "::80", _("National ID") + "::90",
		_("Branch") + ":Link/Branch:120", _("Department") + ":Link/Department:120",
		_("Designation") + ":Link/Designation:120", _("Company") + ":Link/Company:120",
		_("Start Date") + "::80", _("End Date") + "::80", _("Leave Without Pay") + ":Float:70",
		_("Payment Days") + ":Float:120", _("Currency") + ":Link/Currency:80"
	]
	...
	columns += [(e + ":Currency:120") for e in salary_components[_("Earning")]]
	columns += [_("Gross Pay") + ":Currency:120"]
	columns += [(d + ":Currency:120") for d in salary_components[_("Deduction")]]
	columns += [_("Loan Repayment") + ":Currency:120", _("Total Deduction") + ":Currency:120", _("Net Pay") + ":Currency:120"]
...
def get_employee_doj_map():
	...
	for d in frappe.db.sql("""
		SELECT name, date_of_joining, national_id, nssf_no, nhif_no, tax_id FROM `tabEmployee`
		""", as_dict=1):
